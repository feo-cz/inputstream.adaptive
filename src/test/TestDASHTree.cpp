...
TEST_F(DASHTreeTest, TSBMiddlePeriods)
{
  ...
  // NOTE: UTCTiming ignored, mainly to check the number of segments generated
  ...
  EXPECT_EQ(tlPeriod1.GetSize(), 28);
  EXPECT_EQ(tlPeriod1.GetFront()->m_number, 856065392);
...
TEST_F(DASHTreeTest, TSBMiddlePeriodsPastNowTime)
{
  ...
  EXPECT_EQ(tlPeriod1.GetSize(), 0);

  auto& tlPeriod2 =
      tree->m_periods[1]->GetAdaptationSets()[0]->GetRepresentations()[0]->Timeline();

  EXPECT_EQ(tlPeriod2.GetSize(), 0);
}
...
{
  // In this case the manifest has a SegmentTemplate startNumber
  // that could be a result of (period start / SegmentTemplate->duration)
  // NOTE: TSB of 24h, limited by default to 4h, no "availabilityStartTime"
  tree->SetNowTime(1745569648008); // -10 seconds of difference between UTCTiming
  tree->EnableUTCTiming();

  OpenTestFile("mpd/tsb_UTCTiming_direct_1.mpd");

  auto& tlPeriod1 = tree->m_periods[0]->GetAdaptationSets()[0]->GetRepresentations()[0]->Timeline();

  EXPECT_EQ(tlPeriod1.GetSize(), 2400);
  EXPECT_EQ(tlPeriod1.GetFront()->m_number, 290925873);
}

TEST_F(DASHTreeTest, TSBUTCTimingDirect2)
{
  // In this case the manifest has a SegmentTemplate startNumber
  // that is not a result of (period start / SegmentTemplate->duration)
  // NOTE: TSB of 24h, limited by default to 4h, no "availabilityStartTime"
  tree->SetNowTime(1745569676703); // -10 seconds of difference between UTCTiming
  tree->EnableUTCTiming();

  OpenTestFile("mpd/tsb_UTCTiming_direct_2.mpd");

  auto& tlPeriod1 = tree->m_periods[0]->GetAdaptationSets()[0]->GetRepresentations()[0]->Timeline();

  EXPECT_EQ(tlPeriod1.GetSize(), 2400);
  EXPECT_EQ(tlPeriod1.GetFront()->m_number, 290925853);
}

TEST_F(DASHTreeTest, TSBUTCTimingDirect3)
{
  // In this case the manifest has a SegmentTemplate startNumber
  // that is not a result of (period start / SegmentTemplate->duration)
  // NOTE: TSB of 30m, with "availabilityStartTime"
  tree->SetNowTime(1617223977000); // +10 seconds of difference between UTCTiming
  tree->EnableUTCTiming();

  OpenTestFile("mpd/tsb_UTCTiming_direct_3.mpd");

  auto& segments = tree->m_periods[0]->GetAdaptationSets()[0]->GetRepresentations()[0]->Timeline();

  EXPECT_EQ(segments.GetSize(), 450);
  EXPECT_EQ(segments.Get(0)->m_number, 404305539);
}
